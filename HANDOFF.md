...
Currently, authentication works end-to-end, but subsequent transfer and tracking screens use hardcoded mock data.
- **Send Screen** (`android/app/src/main/java/com/payx/app/ui/screens/SendScreen.kt`):
  - Replace hardcoded `sampleRecipients` with `GET /recipients` or live input.
  - Call `GET /api/v1/quote?amount={usdc}` dynamically as the user enters the amount.
  - On confirm, replace `onSubmitted("tx_...")` with a call to `POST /api/v1/payments`.
- **Tracker Screen** (`android/app/src/main/java/com/payx/app/ui/screens/TrackerScreen.kt`):
  - Accept real `paymentId` returned from `POST /api/v1/payments`.
  - Poll `GET /api/v1/payments/:id` to dynamically advance through timeline events (`AWAITING_CONFIRMATION` -> `SETTLEMENT_CONFIRMED` -> `OFFRAMP_PROCESSING` -> `COMPLETED`).
- **Dashboard Screen** (`android/app/src/main/java/com/payx/app/ui/screens/DashboardScreen.kt`):
  - Replace hardcoded balances and transactions with data from `GET /api/v1/payments`.
